...
function filterUnusedVals(
  ...
  if (canSplitCategory) {
    return { domain: customDomain, range: customRange };
  }
...
function setQuantizeDomain(filterVar, data) {
  ...
  let dataMin = Number(
    d3.min(data, d => {
      return d[filterName] ? Number(d[filterName]) : null;
    })
  );
  let dataMax = Number(
    d3.max(data, d => {
